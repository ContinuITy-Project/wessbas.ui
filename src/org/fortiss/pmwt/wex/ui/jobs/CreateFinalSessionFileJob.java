...
public class CreateFinalSessionFileJob implements IJob< Project >
{
	...
	{
		// -- Input files
		this.m_fInputInitialSessionFile = this.m_project.getFile( Project.TAG_FILE_INITIAL_SESSION );
		this.m_fInputStateFilterFile = this.m_project.getFile( Project.TAG_FILE_STATE_FILTER_CONFIGURATION );
		this.m_fInputTimeRangeFilterFile = this.m_project.getFile( Project.TAG_FILE_TIME_RANGE_FILTER_CONFIGURATION );

		// -- Output files
		this.m_fOutputFinalSessionFile = this.m_project.addFile( Project.TAG_FILE_FINAL_SESSION, Project.FILENAME_FINAL_SESSION );
		this.m_fOutputSynopticLogFile = this.m_project.addFile( Project.TAG_FILE_SYNOPTIC_LOG_FILE, Project.FILENAME_SYNOPTIC_LOG );
	}
	...
	{
		// -- State filter
		this.m_stateFilter = SerializationUtils.deserializeFromXML( this.m_fInputStateFilterFile, StateFilter.class );

		// -- State filter / Transition threshold time
		Long lTransitionThresholdTime = this.m_stateFilter.getTransitionThresholdTime();
		if( lTransitionThresholdTime != null && lTransitionThresholdTime.longValue() > 0 )
		{
			this.m_bConsiderTransitionThresholdTime = true;
			this.m_lTransitionThresholdTime = lTransitionThresholdTime.longValue();
		}
		...
		this.m_strSessionMustStartWithStateName = StringUtils.toNull( StringUtils.toEmpty( this.m_stateFilter.getSessionMustStartWithUseCase() ).toLowerCase() );
		this.m_strSessionMustEndWithStateName = StringUtils.toNull( StringUtils.toEmpty( this.m_stateFilter.getSessionMustEndWithUseCase() ).toLowerCase() );

		// -- Time Range filter
		this.m_timeRangeFilter = SerializationUtils.deserializeFromXML( this.m_fInputTimeRangeFilterFile, TimeRangeFilter.class );

		// -- Retrieve time range
		Long lngDataStartTime = (Long)this.m_project.getValue( Project.TAG_VALUE_FIRST_TIMESTAMP );
		Long lngDataStopTime = (Long)this.m_project.getValue( Project.TAG_VALUE_LAST_TIMESTAMP );

		this.m_lRangeFrom = lngDataStartTime.longValue() + TimeUnit.MINUTES.toMillis( this.m_timeRangeFilter.getCropMinutesFromTheLeft() );
		this.m_lRangeTo = lngDataStopTime.longValue() - TimeUnit.MINUTES.toMillis( this.m_timeRangeFilter.getCropMinutesFromTheRight() );

		System.out.println( "- Crop minutes from the left: " + this.m_timeRangeFilter.getCropMinutesFromTheLeft() );
		System.out.println( "- Crop minutes from the right: " + this.m_timeRangeFilter.getCropMinutesFromTheRight() );
		System.out.println( "- Left timestamp: " + lngDataStartTime.longValue() + " -> " + DateUtils.FORMAT_LONG.format( lngDataStartTime.longValue() ) );
		System.out.println( "- Right timestamp: " + lngDataStopTime.longValue() + " -> " + DateUtils.FORMAT_LONG.format( lngDataStopTime.longValue() ) );
		System.out.println( "- Left result: " + this.m_lRangeFrom + " -> " + DateUtils.FORMAT_LONG.format( this.m_lRangeFrom ) );
		System.out.println( "- Right result: " + this.m_lRangeTo + " -> " + DateUtils.FORMAT_LONG.format( this.m_lRangeTo ) );
	}

	/**
	 * Rewrites the initial session file. Rewriting includes filtering by state name, rewriting of state names, and filtering sessions by time range.
	 * 
	 ...
	 * @throws IOException
	 *             Thrown, if there are problems with accessing/reading/writing/... files.
	 */

	private void createFinalSessionFile() throws IOException
	{
		// -- Rewrite using a modified use case mapping
		try( SessionReader reader = new SessionReader(); SessionWriter sessionWriter = new SessionWriter(); SynopticLogWriter synopticLogWriter = new SynopticLogWriter() )
		{
			// -- Synoptic log file
			...
			while( ( session = reader.readSession() ) != null ) // IOException
			{
				...
				if( this.m_strStatesToIgnoreArray != null && containsStateToIgnore( session ) )
				{
				...
				if( this.m_strMandatoryStateNameArray != null && !containsMandatoryStates( session ) )
				{
				...
				if( lSessionStart >= this.m_lRangeFrom && lSessionStop <= this.m_lRangeTo )
				{
					// -- Desired clause
				}
				else
				{
					...
					if( b1 || b2 )
					{
						System.out.println( "[Core] Session ignored due to timerange: " );
						if( b1 )
						{
							System.out.println( "TR.First < TR.From: " + DateUtils.toString( lSessionStart ) + " (" + lSessionStart + ") < " + DateUtils.toString( this.m_lRangeFrom ) + " (" + this.m_lRangeFrom + ")" );
						}
						if( b2 )
						{
							System.out.println( "TR.Last > TR.To: " + DateUtils.toString( lSessionStop ) + " > " + DateUtils.toString( this.m_lRangeTo ) );
						}
				...
				{
					sessionArray = splitSessionByTransitionThresholdTime( session, this.m_lTransitionThresholdTime );
		...
		System.out.println( "Session count: " + this.m_nSessionCount + " (splitted: " + this.m_nAmountOfSplittedSessions + ")" );
		System.out.println( "Ignored (state name to ignore): " + this.m_nIgnoredByStateName + " (" + NumberUtils.toPercent( this.m_nIgnoredByStateName, this.m_nSessionCount ) + ")" );
		System.out.println( "Ignored (missing state name): " + this.m_nIgnoredByMissingMandatoryName + " (" + NumberUtils.toPercent( this.m_nIgnoredByMissingMandatoryName, this.m_nSessionCount ) + ")" );
		System.out.println( "Ignored (time range): " + this.m_nIgnoredByTimeRange + " (" + NumberUtils.toPercent( this.m_nIgnoredByTimeRange, this.m_nSessionCount ) + ")" );
		System.out.println( "Ignored (start and stop state name): " + this.m_nIgnoredByStartAndEndStateName + " (" + NumberUtils.toPercent( this.m_nIgnoredByStartAndEndStateName, this.m_nSessionCount ) + ")\n" );
	}

	/**
	 * Helper method that extracts all states to be ignored from the state filter.
	 * 
	 * @param stateFilter
	 *            Instance of a state filter.
	 * @return String-array that contains all state names that should be ignored.
	 */
	...
	private String[] getStateToIgnoreArray( StateFilter stateFilter )
	{
		...
		Iterator< Map.Entry< String, StateFilter.CState >> iterator = mapState.entrySet().iterator();
		...
		return lstRemoveStateName.size() > 0 ? lstRemoveStateName.toArray( new String[ 0 ] ) : null;
	}
	...
	private String[] getMandatoryStateArray( StateFilter stateFilter )
	{
		...
		return lstMandatoryState.size() > 0 ? lstMandatoryState.toArray( new String[ 0 ] ) : null;
	}
	 ...
	 *            Array that contains all states that should mark the session as ignorable.
	 * @param session
	 *            Session that includes multiple states.
	 * @return true, if a state was found in the session that should be ignored, false otherwise.
	 */
	...
	private boolean containsStateToIgnore( Session session )
	{
		for( String strIgnoreState : this.m_strStatesToIgnoreArray )
		{
			for( State state : session.getStateList() )
			{
				...
				{
					System.out.println( "[Core] Session '" + session.getSessionID() + "' ignored due to state name '" + strIgnoreState + "'." );
					return true;
				}
			}
		}
	...
	private boolean containsFirstAndLastStateName( Session session )
	{
		...
		{
			String strStartUseCase = session.getStateList().get( 0 ).getName().toLowerCase(); // Provoke NullPointerException
			if( !this.m_strSessionMustStartWithStateName.equals( strStartUseCase ) )
			{
				return false;
			}
		}
		...
		{
			String strEndUseCase = session.getStateList().get( session.getStateCount() - 1 ).getName().toLowerCase(); // Provoke NullPointerException
	 ...
	 * @param session
	 *            Current session.
	 * @return true if the session contains all mandator state names, false otherwise.
	 */
	...
	private Session[] splitSessionByTransitionThresholdTime( Session session, long lTransitionThresholdTime )
	{
		...
		for( int i = 0; i < nSplitCounter; i++ )
		{
			...
			State[] arrSubState = Arrays.copyOfRange( arrState, nSplitIndex0, nSplitIndex1 );

			Session newSession = new Session( session.getSessionID() + "_split_" + nSplitIndex0 + "->" + nSplitIndex1 );
