...
public class EditUseCaseMappingFileDialog extends BaseEditUseCaseMappingFileDialog implements Runnable
{
	/**
	 * Serialization.
	 */
	...
	public void setStateFilterFilePath( String strStateFilterFilePath )
	{
		...
		if( fInputStateFilterFile.exists() && fInputStateFilterFile.isFile() )
		{
			try
			{
				this.m_stateFilter = SerializationUtils.deserializeFromXML( fInputStateFilterFile, StateFilter.class );
	...
	public void run()
	{
		this.m_nRowIndex = this.tabUseCaseMapping.convertRowIndexToModel( this.tabUseCaseMapping.getEditingRow() );
		this.m_nColumnIndex = this.tabUseCaseMapping.convertColumnIndexToModel( this.tabUseCaseMapping.getEditingColumn() );
		this.m_objOldCellValue = this.tabUseCaseMapping.getModel().getValueAt( this.m_nRowIndex, this.m_nColumnIndex );
	}
	...
	{
		DefaultTableModel model = (DefaultTableModel)this.tabUseCaseMapping.getModel();
		...
		Map< String, StateFilter.CState > mapUseCaseMapping = this.m_stateFilter.getStateMap();
		Iterator< Map.Entry< String, StateFilter.CState >> iterator = mapUseCaseMapping.entrySet().iterator();
		while( iterator.hasNext() )
		{
			...
			model.addRow( new Object[] { strOldStateName, strNewStateName, bRemove, bMandatory } );
		}

		// -- Set text
		this.txtSessionMustStartWithUseCase.setText( stateFilter.getSessionMustStartWithUseCase() );
		this.txtSessionMustEndWithUseCase.setText( stateFilter.getSessionMustEndWithUseCase() );
		this.txtTransitionThresholdTime.setText( stateFilter.getTransitionThresholdTime() != null ? stateFilter.getTransitionThresholdTime().toString() : "" );
	}
	...
	{
		Object objNewValue = this.tabUseCaseMapping.getValueAt( nRowIndex, nColumnIndex );
		String strKey = (String)this.tabUseCaseMapping.getValueAt( nRowIndex, 0 );
		...
		if( nColumnIndex == 1 )
		{
			...
			{
				JOptionPane.showMessageDialog( this, "Value \"" + strNewValue + "\" contains illegal characters.", "Error", JOptionPane.ERROR_MESSAGE );

				// -- Reset to old value
				this.tabUseCaseMapping.setValueAt( this.m_objOldCellValue, nRowIndex, nColumnIndex );
			}
		}
	...
	protected void btnSaveActionPerformed( ActionEvent evt )
	{
		...
		{
			// -- Handle values
			String strSessionMustStartWithUseCase = StringUtils.toEmpty( this.txtSessionMustStartWithUseCase.getText() );
			this.m_stateFilter.setSessionMustStartWithUseCase( strSessionMustStartWithUseCase );

			String strSessionMustEndWithUseCase = StringUtils.toEmpty( this.txtSessionMustEndWithUseCase.getText() );
			this.m_stateFilter.setSessionMustEndWithUseCase( strSessionMustEndWithUseCase );

			String strTransitionThresholdTime = StringUtils.toEmpty( this.txtTransitionThresholdTime.getText() );
			Long lTransitionThresholdTime = NumberUtils.toLong( strTransitionThresholdTime, null );
			this.m_stateFilter.setTransitionThresholdTime( lTransitionThresholdTime );

			// -- Serialize
			SerializationUtils.serializeToXML( this.m_fStateFilterFile, this.m_stateFilter );
		}
		catch( JAXBException e )
		{
			e.printStackTrace();
	...
	/**
	 * Triggered when the user changes a cell value. See "http://www.camick.com/java/source/TableCellListener.java" for more information.
	 */
