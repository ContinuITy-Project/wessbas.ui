...
public class FileUtils
{
	...
	public static void concat( File fOutputFile, File... arrInputFileArray ) throws IOException
	{
	...
	public static void concatEx( File fOutputFile, File... arrInputFileArray ) throws IOException
	{
		fOutputFile.createNewFile();
	...
	public static File[] retrieveFileArrayByExtension( String strDirectoryPath, final String strFileExtension ) throws IOException
	{
		return ( new File( strDirectoryPath ) ).listFiles( new FileFilter() {
			@Override
			public boolean accept( File file )
			{
				return file.isFile() && file.getName().endsWith( strFileExtension );
			}
		} );
	}
	...
	public static void copyFileToDirectory( File fSourceFile, File fTargetDirectory ) throws IOException
	{
		...
		Files.copy( fSourceFile.toPath(), fTargetDirectory.toPath(), StandardCopyOption.REPLACE_EXISTING );
	}
	 ...
	 * @return String representation of the provided path that ends with a file separator.
	 */

	public static String toPath( File file )
	{
		boolean bValue = file.getPath().endsWith( "/" ) || file.getPath().endsWith( "\\" );
	...
	{
		return strPath.substring( Math.max( strPath.lastIndexOf( "\\" ), strPath.lastIndexOf( "/" ) ) + 1 );
	}
	...
	public static String[] readLines( String strPath, int nLineCount ) throws IOException
	{
	...
	public static String[] readLines( String strPath, int nLineCount, boolean bTrim, boolean bRemoveEmpty ) throws IOException
	{
		...
		try( FileReader fileReader = new FileReader( file ); BufferedReader bufferedReader = new BufferedReader( fileReader ) )
		{
			String strLine = null;
			int nCounter = 0;
			while( ( strLine = bufferedReader.readLine() ) != null && nCounter <= nLineCount )
			{
				if( bTrim )
				{
	...
	public static void storeProperties( File fOutputFile, Properties properties ) throws IOException
	{
		fOutputFile.createNewFile();
	...
	public static Properties restoreProperties( File fInputFile ) throws IOException
	{
	 ...
	 * @throws IOException
	 *             Thrown, if there are problems with accessing/reading/writing/... the file.
	 */
	 ...
	 * @param strPath
	 *            Path to be checked.
	 * @return true if file exists and is a file (not directory), false otherwise.
	 */
	 ...
	 * @return File object representing the path at the command line argument index.
	 */
	...
	public static File getFile( String[] args, int nIndex )
	{
		...
		{
			System.out.println( "Path to file \"" + args[ nIndex ] + "\" not found." );
			System.exit( 0 );
		}
