...
/**
 * Creates a "behaviorModels.properties" file based on the content of a "behaviormix.txt" file.
 */
...
public class BehaviorMixFileToBehaviorModelsFile
{
	...
	public static void create( File fInputBehaviorMixFile, File fOutputBehaviorModelsFile, File fOutputBehaviorModelDirectory ) throws IOException
	{
		fInputBehaviorMixFile.createNewFile();

		List< CBehaviorEntry > lstContent = readBehaviorMixFile( fInputBehaviorMixFile );
		writeBehaviorModelsFile( fOutputBehaviorModelsFile, lstContent, fOutputBehaviorModelDirectory );
	}

	/**
	 * Reads a "behaviormix.txt" file and returns an array of model elements representing each line read.
	 * 
	 * @param fBehaviorMixFile
	 *            "behaviormix.txt" file.
	 * @return Array of {@link org.fortiss.pmwt.wex.ui.tools.BehaviorMixFileToBehaviorModelsFile.CBehaviorEntry} elements.
	 * @throws IOException
	 *             Thrown, if there are problems with accessing/reading/writing/... files.
	 */

	private static List< CBehaviorEntry > readBehaviorMixFile( File fBehaviorMixFile ) throws IOException
	{
		...
		for( String strLine : strLineArray )
		{
			...
			lstBehaviorEntry.add( new CBehaviorEntry( strValue0, Double.parseDouble( strValue2 ), strValue3 ) );
		}
	...
	/**
	 * Writes a "behaviorModels.properties" file based on the content of a "behaviormix.txt" file.
	 * 
	 * @param fOutputBehaviorModelsFile
	...
	private static void writeBehaviorModelsFile( File fOutputBehaviorModelsFile, List< CBehaviorEntry > lstContent, File fOutputBehaviorModelDirectory ) throws IOException
	{
		...
		for( int i = 0; i < lstContent.size(); i++ )
		{
			...
			String strValue0 = FileUtils.toSlashPath( FileUtils.toPath( fOutputBehaviorModelDirectory ) ) + FileUtils.extractFileName( cbe.behaviorModelPath );
	...
	private static final class CBehaviorEntry
	{
		...
		public CBehaviorEntry( String name, double frequency, String behaviorModelPath )
		{
			this.name = name;
